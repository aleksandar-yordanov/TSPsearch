...
class Ant:
    ...
    def __build_probabilites(self,current_index,neighbours):
        pre_probabilities = []
        probabilities = []
        probability_sum = 0
        for city_index, city_distance in enumerate(neighbours):
            if city_distance == -1:
                pre_probabilities.append(0)
                continue
            
            pheromone_on_edge = self.pheromone_matrix[current_index][city_index]
            ...
            pre_probabilities.append(pre_probability)
            probability_sum += pre_probability
        
        for i in pre_probabilities:
            probabilities.append(i/probability_sum)
    ...
    def __move(self,tour):
        ...
        probabilites = self.__build_probabilites(current_position,neighbours)
        running_total = 0
...
def evaluate_pheromone(trails_costs,pheromone_matrix,rho):
    ...
    for trail_costs in trails_costs:
        pheromone_laid = 1/trail_costs[1]
        for i in range(len(trail_costs[0]) - 1):
            pheromone_matrix[trail_costs[0][i]][trail_costs[0][i+1]] += pheromone_laid


def ant_colony_system(num_ants):
    ...
    inital_pheromone = num_cities / greedy_tour[1]
    ...
    while time.time() - start_time < 55:
        ...
        for trail_cost in trails_costs:
            cur_cost = trail_cost[1]
            if cur_cost < best_solution_cost:
                best_solution = trail_cost[0]
                best_solution_cost = cur_cost

        evaluate_pheromone(trails_costs,pheromone_matrix,rho)

    return (best_solution,best_solution_cost)

to_unpack = ant_colony_system(num_ants)
