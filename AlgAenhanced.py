...
Improvements on this algorithm are based on these paper:
https://core.ac.uk/download/pdf/211817601.pdf
...
def member_crossover(parent_x,parent_y):
    # Second enhancement is using partially mapped crossover
    # This was chosen as it is much faster than classical crossover, leading to much more iterations in the time given.
    point1, point2 = sorted(random.sample(range(len(parent_x)-1), 2))
    child_x = parent_x[:point1] + parent_y[point1:point2] + parent_x[point2:]
    child_y = parent_y[:point1] + parent_x[point1:point2] + parent_y[point2:]

    mapping_x = {}
    mapping_y = {}

    for i in range(point1,point1+len(child_x[point1:point2])):
        if child_x[i] != child_y[i]:
            mapping_x[child_x[i]] = child_y[i]
        if child_y[i] != child_x[i]:
            mapping_y[child_y[i]] = child_x[i]
            
    for i in range(point1):
        while child_x[i] in mapping_x:
            child_x[i] = mapping_x[child_x[i]]
        while child_y[i] in mapping_y:
            child_y[i] = mapping_y[child_y[i]]
    
    for i in range(point2,len(child_x)):
        while child_x[i] in mapping_x:
            child_x[i] = mapping_x[child_x[i]]
        while child_y[i] in mapping_y:
            child_y[i] = mapping_y[child_y[i]]

    c1_fitness = get_fitness(child_x)
    c2_fitness = get_fitness(child_y)
...
def mutate(tour, mutation_chance):
    # Third enhancement is a different mutation strategy based on the second paper
    if random.random() < mutation_chance:
        start = random.randint(2, len(tour) - 2)
        end = random.randint(start + 1, len(tour) - 1)
        tour[start:end+1] = reversed(tour[start:end+1])

def roulette_wheel(population,fitness_array,total_fitness):
    selection_point = random.random() * total_fitness
    running_total = 0
...
def genetic_algorithm(pop_size):
    elite_population = generate_population(pop_size*10)
    fitness_scores = []
    for tour in elite_population:
        fitness_scores.append(get_fitness(tour))
    top_indices = heapq.nlargest(pop_size, range(len(fitness_scores)), key=lambda i: fitness_scores[i])
    population = [elite_population[i] for i in top_indices]
    population.pop(-1)
    population.append(get_greedy_tour()[0])
    fittest_individual = (0,0)
    start_time = time.time()
    while time.time() - start_time < 55:
        ...
        local_fittest_individual = max(fitness_scores)
        if local_fittest_individual > fittest_individual[1]:
            fittest_individual = (population[fitness_scores.index(local_fittest_individual)],local_fittest_individual)
        
        new_population = []

        for i in range(pop_size-1):
            parent_x = roulette_wheel(population,fitness_scores,total_fitness)
            parent_y = roulette_wheel(population,fitness_scores,total_fitness)
            child_z = member_crossover(parent_x,parent_y)
            mutate(child_z,0.03)
            child_z = two_opt(child_z)
            new_population.append(child_z)
        
        new_population.append(fittest_individual[0])
    ...
    if local_fittest_individual > fittest_individual[1]:
        fittest_individual = (population[fitness_scores.index(local_fittest_individual)],local_fittest_individual)
    return fittest_individual[0]

tour = genetic_algorithm(pop_size)
