...
def member_crossover(parent_x,parent_y):
    # This is classical crossover, experiment with different crossover mechanisms in enhanced
    crossover_point = random.randint(0,num_cities-1)
    child_x = parent_x[:crossover_point] + parent_y[crossover_point:]
    child_y = parent_y[:crossover_point] + parent_x[crossover_point:]
    list_counter_x = set()
    for i in range(num_cities):
        if child_x[i] in list_counter_x:
            for j in range(num_cities):
                if child_y[j] not in child_x:
                    child_x[i] = child_y[j]
                    break
        
        list_counter_x.add(child_x[i])
    
    list_counter_y = set()
    for i in range(num_cities):
        if child_y[i] in list_counter_y:
            for j in range(num_cities):
                if child_x[j] not in child_y:
                    child_y[i] = child_x[j]
                    break
        
        list_counter_y.add(child_y[i])
    
    child_x_fitness = get_fitness(child_x)
    child_y_fitness = get_fitness(child_y)
    if child_x_fitness > child_y_fitness:
        return child_x
    else:
        return child_y
...
def mutate(tour,mutation_chance):
    if random.random() < mutation_chance:
        random_values = random.sample(range(0,num_cities-1),2)
        tour[random_values[0]],tour[random_values[1]] = tour[random_values[1]],tour[random_values[0]]

def roulette_wheel(population,fitness_array,total_fitness):

    #using roulette wheel approach bound from 0 - total fitness to avoid unecessary computation
    selection_point = random.random() * total_fitness
    running_total = 0
    for i, fitness in enumerate(fitness_array):
        #implement binary search for enhanced
...
def genetic_algorithm(pop_size):
    ...
    curFittest = (0,0) #make to store best 
    start_time = time.time()
    ...
    fitest_individual = population[fitness_scores.index(max(fitness_scores))]
